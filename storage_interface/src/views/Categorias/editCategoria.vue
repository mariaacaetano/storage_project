...
export default {
  ...
  setup() {
    ...
    const searchQuery = ref(""); // Barra de pesquisa
    const categoria = ref({
      nome: '',
      descricao_categoria: '',
      localizacao: '',
    ...
    const categoriaId = route.params.id; // ID da categoria passada pela URL
    ...
    const carregarCategoria = async () => {
      try {
        ...
        if (response.status === 200) {
          categoria.value = response.data; // Atualiza os dados da categoria no front-end
        }
      } catch (error) {
    ...
    const searchCategorias = async () => {
      try {
        ...
        // Realize algo com os dados de resposta da pesquisa, se necessário
      } catch (error) {
    ...
    // Carregar os dados da categoria quando o componente for montado
    onMounted(() => {
      carregarCategoria(); // Chama a função de carregamento no momento em que o componente é montado
    });

    return { categoria, searchQuery, salvar, voltar, searchCategorias };
  },
};
</script>
